    ...
    <main class="container max-w-xl mx-auto space-y-8 mt-8 px-2 md:px-0 min-h-screen">
        @if($posts->isEmpty())
            <div class="flex justify-center items-center">
                @guest
                    <div class="text-center py-12 px-24 border border-sky-200 rounded-xl">
                        <h1 class="text-2xl justify-center items-center">
                            Welcome to
                            <span class="text-sky-800 block text-6xl bold my-4">{{ config('app.name') }}</span>
                        </h1>
                        <p class="mt-8 "><a href="{{ route('login') }}" class="text-blue-600">Log in</a> to create your first
                            post!</p>
                    </div>
                @endguest
            </div>
